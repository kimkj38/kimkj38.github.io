...
GAN의 Global Optimum point는 G가 고정되어 있을 때 D*G(x) = Pdata(x)/Pdata(x)+Pg(x)이며 Pg=Pdata인데 이 두 조건이 만족할 때 D(G(z))는 1/2로 수렴한다. 이 식을 도출하는 자세한 증명과정 나동빈님의 유튜브 강의에서 볼 수 있다.  
